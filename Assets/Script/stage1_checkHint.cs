...
//hint에 붙음
public class stage1_checkHint : MonoBehaviour
{
    ...
    void OnMouseDown()
    {
        if (stage1_submitClick.hintCount < 3)
        {
            if (!hintOn)
            {
                //hint count
                switch(stage1_submitClick.hintCount){
                    case 0:
                ...
                // hintCount++;
                stage1_submitClick.hintCount++; //힌트=>plating당 한번만 가능
